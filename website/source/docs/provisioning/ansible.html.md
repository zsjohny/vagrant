...
~> **Warning:** If you are not familiar with Ansible and Vagrant already, I
recommend starting with the [shell provisioner](/docs/provisioning/shell.html).
However, if you are comfortable with Vagrant already, Vagrant is a great way to
learn Ansible.
...
You can of course target other operating systems that do not have YUM by changing the
...
It is good to know that the following Ansible settings always override the `config.ssh.username` option defined in [Vagrant SSH Settings](/docs/vagrantfile/ssh_settings.html):
